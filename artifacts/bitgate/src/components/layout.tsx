...
import { Wallet, LogOut, TerminalSquare, Hexagon } from "lucide-react";
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            <Link href="/" className="flex items-center gap-3 group">
              <img src="/images/bitgate-logo.png" alt="BitGate" className="h-9 w-auto" />
            </Link>
            ...
            <nav className="hidden md:flex items-center gap-8">
              ...
              <Link
                href="/dashboard"
                className={`text-sm font-medium transition-colors hover:text-primary ${
                  location === "/dashboard" ? "text-primary text-glow" : "text-muted-foreground"
                }`}
              >
                Dashboard
              </Link>
              ...
              {isConnected && address ? (
                <div className="flex items-center gap-3">
                  <div className="hidden sm:flex flex-col items-end">
                    <span className="text-xs text-muted-foreground font-medium uppercase tracking-wider">Connected</span>
                    <span className="text-sm font-mono text-primary bg-primary/10 px-2 py-0.5 rounded-md border border-primary/20">
                      {truncateAddress(address)}
                    </span>
                  </div>
                  <Button variant="outline" size="icon" onClick={disconnect} className="rounded-full" title="Disconnect">
      ...
      <footer className="border-t border-white/5 bg-card/30 mt-auto">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <p className="text-sm text-muted-foreground/60">
            &copy; {new Date().getFullYear()} BitGate Protocol. Decentralized API Access.
          </p>
        </div>
      </footer>
    </div>
